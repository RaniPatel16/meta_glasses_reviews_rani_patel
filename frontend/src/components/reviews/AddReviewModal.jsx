...
const AddReviewModal = ({ open, onClose }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      reviewID: Yup.string().required('Required').matches(/^rev-\w+$/i, 'Must start with rev-'),
    ...
    onSubmit: async (values, { resetForm }) => {
      const payload = {
        ...
        user: user?._id || user?.id || '000000000000000000000000' // fallback if no user
      };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth PaperProps={{ className: 'glass rounded-2xl' }}>
      <DialogTitle className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700">
        <span className="font-bold text-slate-800 dark:text-white">Add New Review</span>
      ...
      <form onSubmit={formik.handleSubmit}>
        <DialogContent className="space-y-4 pt-6">
          <div className="flex gap-4">
            <TextField
              fullWidth label="Review ID" name="reviewID"
              value={formik.values.reviewID} onChange={formik.handleChange}
              error={formik.touched.reviewID && Boolean(formik.errors.reviewID)}
              helperText={formik.touched.reviewID && formik.errors.reviewID}
            />
            <TextField
        ...
        <DialogActions className="p-4 border-t border-slate-200 dark:border-slate-700">
          <Button onClick={onClose} disabled={actionLoading} className="text-slate-600 dark:text-slate-400">Cancel</Button>
          <Button type="submit" variant="contained" disabled={actionLoading} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-6">
            {actionLoading ? <CircularProgress size={24} color="inherit" /> : 'Create Review'}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
